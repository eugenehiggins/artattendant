		...
		<div class="author">
			<span class="posted-on">
				...
				<?php
					...
					printf( esc_html__( 'By %1$s on %2$s' ), $name, $time ); ?>
			</span>
		</div>
	</header><!-- /conversation_header -->

	<a href="<?php echo get_permalink( $post_id ) ?>" class="button hsd_goback"><?php esc_attr_e( 'Go back' ) ?></a>
			...
			<?php if ( $data['type'] != 'lineitem' ) : ?>
				<div class="panel panel-default">
					<div class="panel-heading clearfix">
						...
						<h3 class="panel-title pull-right">
							<?php
								...
								printf( esc_html__( '%1$s on %2$s', 'help-scout-desk' ), $name, $time ); ?>
						</h3>
					</div>
					<div class="panel-body">
						<div class="conversation_body clearfix">
							<div class="message">
								<?php echo wpautop( self::linkify( __( $data['body'] ) ) ); ?>
							</div>
					...
					<div class="panel-footer">
						<?php
							printf( esc_html__( '<b>%1$s</b> of %2$s', 'help-scout-desk' ), $thread, esc_attr( $item['threadCount'] ) ); ?>
					</div>
				</div>
			...
			<?php else : ?>
				<div class="line_item">
					<?php
						$name = esc_attr( $data['createdBy']['firstName'] ) . ' ' . esc_attr( $data['createdBy']['lastName'] );
						$time = '<time datetime="'.esc_attr( $data['createdAt'] ).'">'.date_i18n( get_option( 'date_format' ), strtotime( esc_attr( $data['createdAt'] ) ) ).'</time>';
						$status = sprintf( '<span class="label label-%1$s">%2$s</span>', hsd_get_status_class( $data['status'] ), hsd_get_status_label( $data['status'] ) );
						printf( esc_html__( '%1$s by %2$s on %3$s', 'help-scout-desk' ), $status, $name, $time ); ?>
				</div>
			<?php endif ?>
		<?php
		endforeach; ?>
