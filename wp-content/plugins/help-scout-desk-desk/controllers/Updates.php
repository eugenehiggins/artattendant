 ...
 * @subpackage Updates
 */
class HSD_Updates extends HSD_Controller {
	const LICENSE_KEY_OPTION = 'hsd_license_key';
	const LICENSE_STATUS = 'hsd_license_status';
	...
	public static function init() {
		...
		if ( is_admin() ) {
			...
			// AJAX
			add_action( 'wp_ajax_hsd_activate_license',  array( __CLASS__, 'maybe_activate_license' ), 10, 0 );
			add_action( 'wp_ajax_hsd_deactivate_license',  array( __CLASS__, 'maybe_deactivate_license' ), 10, 0 );
			add_action( 'wp_ajax_hsd_check_license',  array( __CLASS__, 'maybe_check_license' ), 10, 0 );
		}
	}
	...
	public static function init_edd_udpater() {
		...
		$edd_updater = new EDD_SL_Plugin_Updater_SA_Mod( self::PLUGIN_URL, self::PLUGIN_FILE, array(
				'version' 	=> self::HSD_VERSION,			// current version number
	...
	public static function register_settings() {
		// Settings
		$settings = array(
			'hsd_activation' => array(
				'title' => __( 'Help Scout Desk Activation', 'help-scout-desk' ),
				'weight' => 0,
				'callback' => array( __CLASS__, 'update_setting_description' ),
				'settings' => array(
					self::LICENSE_KEY_OPTION => array(
						'label' => __( 'License Key', 'help-scout-desk' ),
						'option' => array(
							'type' => 'bypass',
							'output' => self::license_key_option(),
							'description' => sprintf( __( 'Enter your license key to enable automatic plugin updates. Find your license key in your Sprout Apps Dashboard under the <a href="%s" target="_blank">Downloads</a> section.', 'help-scout-desk' ), self::PLUGIN_URL.'/account/' ),
							),
						),
					),
				),
	...
	public static function license_key_option() {
		ob_start(); ?>
			...
			<?php if ( self::$license_status != false && self::$license_status == 'valid' ) : ?>
				<button id="hsd_activate_license" class="button" disabled="disabled"><?php _e( 'Activate License', 'help-scout-desk' ) ?></button> 
				<button id="hsd_deactivate_license" class="button"><?php _e( 'Deactivate License', 'help-scout-desk' ) ?></button>
			<?php else : ?>
				<button id="hsd_activate_license" class="button button-primary"><?php _e( 'Activate License', 'help-scout-desk' ) ?></button>
			<?php endif ?>
	...
	public static function update_setting_description() {
		// _e( 'TODO Describe the license key and how to purchase.', 'help-scout-desk' );
	}
	...
	public static function maybe_activate_license() {
		...
		$message = ( $activated ) ? __( 'Thank you for supporting the future of Help Scout Desk and Sprout Apps.', 'help-scout-desk' ) : __( 'License is not active.', 'help-scout-desk' );
		$response = array(
				'activated' => $activated,
				'response' => $message,
				'error' => ! $activated,
	...
	public static function maybe_deactivate_license() {
		...
		$message = ( $deactivated ) ? __( 'License is deactivated.', 'help-scout-desk' ) : __( 'Something went wrong. Contact support for help.', 'help-scout-desk' );
		$response = array(
				'valid' => $deactivated,
				'response' => $message,
				'error' => ! $deactivated,
	...
	public static function maybe_check_license() {
		...
		$message = ( $is_valid ) ? __( 'Thank you for supporting the future of Help Scout Desk and Sprout Apps.', 'help-scout-desk' ) : __( 'License is not valid.', 'help-scout-desk' );
		$response = array(
				'valid' => $is_valid,
				'response' => $message,
			);
