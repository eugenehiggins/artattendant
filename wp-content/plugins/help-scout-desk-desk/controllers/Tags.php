...
class HSD_Tags extends HSD_Controller {
		...
		foreach ( self::$tags as $tag_id ) {
			...
			if ( isset( $hs_tags[ $tag_id ] ) ) {
				...
				if ( ( array_search( $tag_name, $conversation['tags'] ) ) !== false ) {
					$conversation_tags[ $tag_id ] = $tag_name;
				}
			}
		}

		return apply_filters( 'hsd_get_converstation_tags', $conversation_tags, $conversation, $hs_tags );
	...
	public static function converstation_tags( $conversation = array() ) {
		...
		print self::load_view_to_string( 'section/conversation-tags', array(
				...
				'converation_tags' => $conversation_tags, // backwards compat
	...
	public static function show_tag_options() {
		...
		$view .= sprintf( '<p class="description">%s</p>', esc_html__( 'Select the tags your customers have available to choose from.', 'help-scout-desk' ) );

		$view .= sprintf( '<p><label><input type="checkbox" name="hsd_reset_tag_cache" value="1" id="hsd_reset_tag_cache" />%s</label></p>', esc_html__( 'Sync tags with the Help Scout', 'help-scout-desk' ) );
		return $view;
	}
