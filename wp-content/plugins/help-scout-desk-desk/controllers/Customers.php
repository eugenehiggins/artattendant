...
class HSD_Customers extends HSD_Controller {
	...
	public static function show_sync_options() {
		$button = sprintf( '<button id="hsd_sync_customers" class="button">%s</button>', esc_html__( 'Sync Users', 'help-scout-desk' ) );
		ob_start();
		?>
		...
		<script type="text/javascript">
			...
			jQuery("#hsd_sync_customers").on('click', function(event) {
				...
				if( confirm( '<?php esc_html_e( 'Are you sure? This process could take a while so you may want to go get some coffee.', 'help-scout-desk' ) ?>' ) ) {
	...
	public static function maybe_init_sync() {
		...
		if ( ! isset( $_POST['in_progress'] ) ) {
			wp_send_json_error( esc_html__( 'No progress!', 'help-scout-desk' ) );
		}
		...
		if ( ! $in_progress ) {
			wp_send_json_success( array(
				'message' => sprintf( esc_html__( 'Starting to sync %s users...', 'help-scout-desk' ), $user_total ),
				'progress' => 5,
				'user_sync_count' => 1,
			) );
		...
		if ( $new_progress === -1 ) {
			wp_send_json_success( array(
				'message' => sprintf( esc_html__( 'It happened, %s users are now in sync at Help Scout!', 'help-scout-desk' ), $user_total ),
				'progress' => 100,
				'user_sync_count' => 1,
			) );
			return; // not necessary but whatevs
		...
		// to be continued...
		wp_send_json_success( array(
			'message' => sprintf( esc_html__( 'Some magic in the background has completed %1$s of the %2$s users...', 'help-scout-desk' ), $new_progress, $user_total ),
